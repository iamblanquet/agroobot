...
export default function CrewPicker({ employees, selection, onChange, loading, disabled }) {
  ...
  return (
    <div className="space-y-3">
      <p className="text-xs text-slate-500 dark:text-slate-400">Busca por nombre o puesto y añade a cada persona en el rol que desempeñará hoy. Cada persona cuenta una sola vez.</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        {Object.entries(roles).map(([role, label]) => {
          ...
          const matches = available.filter(employee => normalize(`${employee.nombre} ${employee.puesto}`).includes(query));
          return <section key={role} className="p-3 rounded-2xl bg-slate-50 dark:bg-slate-950 border border-[#e2ebd3] dark:border-[#253905] space-y-3 min-w-0">
            ...
            <div className="relative">
              ...
              <input aria-label={`Buscar ${label.toLowerCase()}`} disabled={loading || disabled} value={queries[role]} onChange={event => setQueries(previous => ({ ...previous, [role]: event.target.value }))} onKeyDown={event => { if (event.key === 'Enter') event.preventDefault(); }} placeholder="Nombre o puesto…" className="w-full pl-8 pr-2 py-2.5 text-xs rounded-xl bg-white dark:bg-[#152202] border border-slate-300 dark:border-slate-700 focus:outline-none focus:ring-2 focus:ring-[#a1c62e]" />
            </div>
            {loading ? <p role="status" className="text-xs text-slate-500">Cargando empleados…</p> : <ul aria-label={`${label} disponibles`} className="max-h-40 overflow-y-auto space-y-1">
              {matches.slice(0, 20).map(employee => <li key={employee.id}><button type="button" disabled={disabled} onClick={() => add(role, employee)} aria-label={`Añadir a ${employee.nombre} como ${label.toLowerCase()}`} className="w-full text-left flex items-center justify-between gap-2 p-2 rounded-lg hover:bg-[#a1c62e]/20 disabled:opacity-50">
                <span className="min-w-0"><span className="block text-xs font-semibold break-words">{employee.nombre}</span><span className="block text-[11px] text-slate-500 dark:text-slate-400 break-words">{employee.puesto}</span></span><Plus className="w-4 h-4 shrink-0" />
              </button></li>)}

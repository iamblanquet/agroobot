  ...
  return (
    <div className="max-w-7xl mx-auto px-3 sm:px-6 py-6 pb-24 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 dark:border-slate-800 pb-4">
        <div>
          <h2 className="text-lg font-bold text-slate-900 dark:text-white flex items-center gap-2">
            <Shield className="w-5 h-5 text-amber-600 dark:text-amber-400" /> Administración IT & Control de Accesos
          </h2>
        ...
        <div className="flex items-center gap-2">
          <button
            type="button"
            onClick={loadAll}
            className="p-2 rounded-xl bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 text-xs font-semibold flex items-center gap-1.5 transition shadow-sm"
          >
          ...
          <button
            ...
            className="px-3.5 py-2 rounded-xl bg-amber-600 hover:bg-amber-500 text-white text-xs font-bold flex items-center gap-1.5 transition shadow-md shadow-amber-600/30"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="p-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 flex items-center gap-3 shadow-sm">
          <div className="p-3 rounded-xl bg-emerald-50 dark:bg-emerald-950 text-emerald-700 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-700/40">
            <Server className="w-5 h-5" />
          </div>
          <div>
            <p className="text-xs text-slate-500 dark:text-slate-400 font-medium">Estado del Backend</p>
            <p className="text-sm font-bold text-emerald-700 dark:text-emerald-400">
              {health?.status === 'OK' ? '✅ Standalone API Activo' : 'Conectando...'}
            </p>
            <p className="text-[10px] text-slate-400 font-mono">SQLite Relacional (15 tablas)</p>
          </div>
        </div>

        <div className="p-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 flex items-center gap-3 shadow-sm">
          <div className="p-3 rounded-xl bg-purple-50 dark:bg-purple-950 text-purple-700 dark:text-purple-400 border border-purple-200 dark:border-purple-700/40">
            <Activity className="w-5 h-5" />
          </div>
          <div>
            <p className="text-xs text-slate-500 dark:text-slate-400 font-medium">Acceso & Offline Storage</p>
            <p className="text-sm font-bold text-purple-700 dark:text-purple-400">PIN 4 Dígitos + IndexedDB</p>
            <p className="text-[10px] text-slate-400">Sesión persistente y sync idempotente</p>
          </div>
        </div>

        <div className="p-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 flex items-center gap-3 shadow-sm">
          <div className="p-3 rounded-xl bg-sky-50 dark:bg-sky-950 text-sky-700 dark:text-sky-400 border border-sky-200 dark:border-sky-700/40">
            <Clock className="w-5 h-5" />
          </div>
          <div>
            <p className="text-xs text-slate-500 dark:text-slate-400 font-medium">Alertas Automáticas Cron</p>
            <p className="text-sm font-bold text-sky-700 dark:text-sky-400">21:00 · 21:30 · 08:00</p>
            <p className="text-[10px] text-slate-400">Zona Horaria: America/Merida</p>
          </div>
        </div>
      </div>

      {/* DISPARADORES MANUALES DE ALERTAS CRON */}
      <div className="p-4 rounded-2xl bg-white dark:bg-slate-900/90 border border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 shadow-sm">
        <div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          <button
            ...
            className="px-2.5 py-1.5 rounded-lg bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-slate-200 text-xs font-bold flex items-center gap-1 transition"
          >
            <Play className="w-3 h-3 text-amber-600 dark:text-amber-400" /> Probar 21:00 (Obras Sin Reporte)
          </button>
          <button
            type="button"
            ...
            className="px-2.5 py-1.5 rounded-lg bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-slate-200 text-xs font-bold flex items-center gap-1 transition"
          >
            <Play className="w-3 h-3 text-sky-600 dark:text-sky-400" /> Probar 21:30 (Corte Tablero)
          </button>
          <button
            type="button"
            ...
            className="px-2.5 py-1.5 rounded-lg bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-slate-200 text-xs font-bold flex items-center gap-1 transition"
          >
            <Play className="w-3 h-3 text-emerald-600 dark:text-emerald-400" /> Probar 08:00 (Incidencias/300h)
          </button>
        </div>

        {cronTriggerStatus && (
          <span className="text-xs font-mono text-sky-800 dark:text-sky-300 bg-sky-50 dark:bg-sky-950/80 px-2.5 py-1 rounded-md border border-sky-300 dark:border-sky-700/50">
            {cronTriggerStatus}
          </span>
        )}
      </div>
      ...
      <div className="p-5 sm:p-6 rounded-2xl bg-white dark:bg-slate-900/90 border border-slate-200 dark:border-slate-800 shadow-sm dark:shadow-xl space-y-4">
        <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
          ...
          <span className="text-xs text-slate-500 dark:text-slate-400 font-mono">{users.length} usuarios registrados</span>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs text-slate-700 dark:text-slate-300">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {users.map((u) => {
                const roleColors = {
                  campo: 'bg-emerald-50 dark:bg-emerald-950 text-emerald-800 dark:text-emerald-300 border-emerald-300 dark:border-emerald-700',
                  supervisor: 'bg-blue-50 dark:bg-blue-950 text-blue-800 dark:text-blue-300 border-blue-300 dark:border-blue-700',
                  direccion: 'bg-purple-50 dark:bg-purple-950 text-purple-800 dark:text-purple-300 border-purple-300 dark:border-purple-700',
                  it: 'bg-amber-50 dark:bg-amber-950 text-amber-800 dark:text-amber-300 border-amber-300 dark:border-amber-700'
                };

                return (
                  <tr key={u.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition">
                    <td className="py-3 px-4 font-mono font-bold text-slate-900 dark:text-white">{u.username}</td>
                    <td className="py-3 px-4">{u.nombre}</td>
                    <td className="py-3 px-4">
                      <span className={`px-2 py-0.5 rounded text-[10px] font-bold border uppercase ${roleColors[u.rol] || 'bg-slate-100 dark:bg-slate-800'}`}>
                        {u.rol}
                      </span>
                    </td>
                    <td className="py-3 px-4 text-center">
                      {u.pin ? (
                        <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-md bg-amber-50 dark:bg-amber-950/70 border border-amber-300 dark:border-amber-600/40 text-amber-800 dark:text-amber-300 font-mono font-bold text-xs shadow-sm">
                          <KeyRound className="w-3 h-3 text-amber-600 dark:text-amber-400" /> {u.pin}
                        </span>
                      ) : (
                    ...
                    <td className="py-3 px-4 text-right">
                      <button
                        ...
                        className="px-3 py-1 rounded-lg bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-slate-300 text-xs font-semibold transition"
                      >
                        Editar / PIN
                      </button>
                    </td>
          ...
          <div className="divide-y divide-slate-100 dark:divide-slate-800 max-h-56 overflow-y-auto">
            {predios.map((pr) => (
              <div key={pr.id} className="py-2.5 flex items-center justify-between text-xs">
                <div>
                  <p className="font-bold text-slate-800 dark:text-slate-200">{pr.nombre}</p>
                  <p className="text-[10px] text-slate-500">{pr.regimen} • Útil: {pr.superficie_util_ha} ha / Legal: {pr.superficie_legal_ha} ha</p>
                </div>
                <span className="px-2 py-0.5 rounded text-[10px] font-mono bg-slate-100 dark:bg-slate-950 text-slate-600 dark:text-slate-400 border border-slate-200 dark:border-slate-800">
                  GeoJSON OK
                </span>
              </div>
            ))}
          ...
          <div className="divide-y divide-slate-100 dark:divide-slate-800 max-h-56 overflow-y-auto">
            {obras.map((o) => (
              <div key={o.id} className="py-2.5 flex items-center justify-between text-xs">
                <div>
                  <p className="font-bold text-slate-800 dark:text-slate-200">{o.nombre}</p>
                  <p className="text-[10px] text-slate-500">{o.proyecto_nombre} • Fase: {o.fase_actual}</p>
                </div>
                <span className="px-2 py-0.5 rounded text-[10px] uppercase font-bold bg-blue-50 dark:bg-blue-950 text-blue-800 dark:text-blue-300 border border-blue-300 dark:border-blue-800">
                  {o.estado}
                </span>
              </div>
            ))}
        ...
        <div className="fixed inset-0 bg-slate-950/60 dark:bg-slate-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-3 sm:p-4">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-3xl max-w-md w-full max-h-[90vh] overflow-y-auto p-5 sm:p-6 shadow-2xl space-y-4">
            <div className="flex justify-between items-center border-b border-slate-100 dark:border-slate-800 pb-3">
              ...
              <button type="button" onClick={() => setShowCreateModal(false)} className="text-slate-400 hover:text-slate-900 dark:hover:text-white">✕</button>
            </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="ej. 1234"
                    className="w-full px-3 py-2 rounded-xl bg-slate-50 dark:bg-slate-950 border border-amber-400 dark:border-amber-600/40 text-amber-800 dark:text-amber-300 font-mono font-bold text-xs focus:outline-none"
                  />
                </div>
              </div>
        ...
        <div className="fixed inset-0 bg-slate-950/60 dark:bg-slate-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-3 sm:p-4">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-3xl max-w-md w-full max-h-[90vh] overflow-y-auto p-5 sm:p-6 shadow-2xl space-y-4">
            <div className="flex justify-between items-center border-b border-slate-100 dark:border-slate-800 pb-3">
              ...
              <button type="button" onClick={() => setEditingUser(null)} className="text-slate-400 hover:text-slate-900 dark:hover:text-white">✕</button>
            </div>
            ...
            <form onSubmit={handleEditUserSubmit} className="space-y-3">
              ...
              <div className="flex items-center gap-2 pt-2">
                ...
                />
                <label htmlFor="activoCheck" className="text-xs font-medium text-slate-700 dark:text-slate-200">Usuario Activo con Permisos</label>
              </div>
